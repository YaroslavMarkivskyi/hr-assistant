...
class TimeOffMapper:
    ...
    1. Raw User Text -> Structured Data (via AI)
    2. Service/Domain Objects -> UI View Models
    """
    ...
    @staticmethod
    async def map_to_leave_form_data(ctx: IntentContext) -> LeaveRequestFormViewModel:
        """
        Uses AI (2nd pass) to extract leave details from text 
        and prepares data for the Request Form (Adaptive Card).
        """
        try:
            current_date = ctx.container.time_service.now_utc().strftime("%Y-%m-%d")
        ...
        entities: TimeOffExtractionParams = await ctx.container.ai_service.extract_data(
            user_text=ctx.ctx.activity.text,
            result_type=TimeOffExtractionParams,
            prompt_key=PromptKeys.TIMEOFF_EXTRACT,
            context=context_str
        )
        
        logger.info(f"🧠 [Mapper] Extracted TimeOff entities: {entities}")

        start_str = entities.start_date.isoformat() if entities.start_date else None
        end_str = entities.end_date.isoformat() if entities.end_date else None
        
        return LeaveRequestFormViewModel(
            default_type=entities.leave_type,
